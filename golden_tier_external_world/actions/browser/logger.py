"""
BROWSER_MCP_SKILL — Audit Logger
Phase 1: Append-only JSONL log under 70-LOGS/browser/YYYY-MM-DD.jsonl.

Constitution compliance:
  - Section 7: All skills must log to 70-LOGS/
  - Principle VI: Fail Safe — logger never raises; failures are swallowed
  - Section 8: No secrets in logs — BrowserResult carries no credentials
"""
...
from .models import BrowserRequest, BrowserResult


class BrowserLogger:
    """
    Writes append-only JSONL records to 70-LOGS/browser/YYYY-MM-DD.jsonl.

    Every public method is non-throwing — logging failures must not
    interrupt the browser action flow.
    """

    def __init__(self, vault_root: str | Path = ".") -> None:
        self._vault   = Path(vault_root)
        self._log_dir = self._vault / "70-LOGS" / "browser"

    # ------------------------------------------------------------------
    # Public API
    # ------------------------------------------------------------------

    def log_submitted(self, request: BrowserRequest) -> None:
        self._append({
            "event":      "submitted",
    ...
    def log_queued_for_hitl(self, request: BrowserRequest, hitl_request_id: str) -> None:
        self._append({
    ...
    def log_result(self, result: BrowserResult) -> None:
        self._append({
            "event":       "result",
